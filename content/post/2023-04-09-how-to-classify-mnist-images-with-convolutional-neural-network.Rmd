...
Importantly, a `convnet`  takes tensors of shape (image_height, image_widht, image_channels), not including the
...
as the a `depth` axis. For an RGB image, the dimention of the depth axis is 3 with Red, green and blue channels.
...
an `output feature map`. This out put feature map is still 3D: width, height and depth. But the depth now is an arbitray 
parameter of the layer and it does not represent the RGB colors: they are called filters. 

Filters encode specific aspects of teh input data. At a higher level, a single filter can encode the concept "presence of a
face in the image".
...
Each of those 32 output channels contains a 26 x 26 grid of values, which is a response map of the fitler over the input, indicating 
...
dimention becomes 26 x 26 (Think how many 3x3 patches you can get from a 28x28 grid.)

* Depth of the output feature map: the number of filters computed by the convolution. We used 32 and ended with 64.


Max-pooling consists of extracting windows from the output feature maps and outputing the max value of each channel. Instead of transforming local patches
via a learned linear transformation, they are transformed via a hardcoded max tensor operation. The max pooling is usually done with 2x2 window.
...
max pooling reduces it to half: 14 x 14, but the second 2d keeps it 14x 14, then max pooling reduced it to half again to 7x7 etc etc.
