...
### New target identification (where AI helps, and where it does not)
...
Most marketed cancer drugs go after well-characterized oncogenes: HER2, EGFR, KRAS. TP53 is a tumor suppressor, the most frequently mutated gene in cancer, and we still do not have a good drug against it. MYC is the other obvious one. It is an oncogenic transcription factor, dysregulated in a huge fraction of human cancers, and "undruggable" for the same reason TP53 is: it does not have a defined ligand-binding pocket. We have been chasing MYC for decades. Cell cycle regulators are a related, long-studied family: CDK4/6, CDK7, CDK9, WEE1, ATR, PARP1/2, the whole DNA damage response (DDR) landscape. We have known about these for a long time. Some we drugged (CDK4/6 inhibitors are now standard of care in HR+ breast cancer; PARP inhibitors are approved across BRCA-mutated cancers). Many we are still trying to drug. A recent [*Cell* review by Yap, Manning, Sapra, Mills, and O'Connor](https://doi.org/10.1016/j.cell.2026.03.035) lists dozens of DDR programs currently in Phase 1 or 2 (sarudaparib, gartisertib, lunresertib, prexasertib, and many more). None of these targets are AI discoveries. We have a lot of work left on targets we have known about for 20+ years.

What is genuinely new is the modality. PROTACs degrade a target instead of inhibiting it. RIPTACs (regulated induced proximity targeting chimeras) recruit a co-essential protein to enforce tumor selectivity. ADCs (antibody-drug conjugates) deliver a cytotoxic payload preferentially to cells expressing a tumor antigen. The ADC point is worth dwelling on because it solves a specific kind of problem: some of the best cancer targets are essential for normal tissue function, which makes them too toxic to drug systemically. Linking a toxic payload to an antibody against a tumor-restricted antigen turns a non-starter target into a viable one. The therapeutic window comes from the delivery, not from the payload. That is also where AI for target identification can actually help — finding the right tumor antigen for the antibody side of the conjugate, or surfacing a co-essential protein for a RIPTAC pair. The new modalities widen the addressable target space; AI can help map it.

The exception worth flagging is [GV20 Therapeutics](https://gv20tx.com/pipeline). My mentor [Shirley Liu](https://en.wikipedia.org/wiki/Xiaole_Shirley_Liu) is the co-founder and CEO. Their AI platform decodes patient B-cell repertoires to discover novel target-antibody pairs simultaneously, and it surfaced IGSF8 as an innate immune checkpoint, a target with very little prior literature attention. They published the discovery in [Cell in 2024](https://pubmed.ncbi.nlm.nih.gov/38657602/). The biology has continued to expand: Shirley told me they have since shown IGSF8 also suppresses dendritic cell antigen presentation, and the follow-up manuscript along with the Phase 1 clinical results have received positive reviews at Nature Medicine.

Their lead antibody GV20-0251 is, as far as I can tell, the world's first AI-designed antibody against an AI-discovered target to enter the clinic. It is in a Phase 1/2 trial (NCT05669430) in advanced solid tumors, with a clinical collaboration to test it in combination with Merck's KEYTRUDA. The most striking data so far is in cutaneous melanoma patients with primary resistance to anti-PD-1: three confirmed partial responses as monotherapy, two of them in patients with liver metastases (a population that is especially hard to treat). Earlier readouts were [presented at ASCO 2025](https://ascopubs.org/doi/10.1200/JCO.2025.43.16_suppl.2531), and the full Phase 1 results were presented at AACR 2026 and highlighted in the closing session. So my prior on AI target ID is not that it is impossible. It is that the bar to do it well is high, and the trick is what you extract from the data. GV20 does start from public RNA-seq, but they apply the [TRUST4](https://github.com/liulab-dfci/TRUST4) algorithm to reconstruct B-cell receptor (BCR) sequences from those reads, which gives them millions of patient-derived BCRs to learn from. The novelty is the modality they pulled out of standard data, not access to some private dataset.
...
- Yap, Manning, Sapra, Mills, O'Connor. Targeting genomic instability in cancer. [*Cell* 189, April 16, 2026](https://doi.org/10.1016/j.cell.2026.03.035) — review of DDR targets, ADCs, and radioconjugates
